...
**Revision:** Version 5 — verified interface-name update, active-profile static management IPs, original walkthrough and diagrams retained  
...
**Known from your `ctrl01` output:** `ens33` currently uses DHCP address
`192.168.0.180/24`; `ens37`, `ens38`, and `ens39` are present without IPv4
addresses. The mapping of those three NICs to VMnet2/3/4 is a **target**, not
proof of their VMware connections. Verify their MAC addresses in Section 7.
The other five nodes must each pass the same check before using their
host-specific commands in Section 9.

### What each network does, and why OVN uses tunnels

| Network | Linux/VyOS device | Actual job | Gateway on the host? |
|---|---|---|---|
| VMnet0 `192.168.0.0/24` | All nodes `ens33`; VyOS `eth0` | SSH, Ansible, OpenStack internal API VIP `.49`; VyOS outside `.60`. | **Yes**: CentOS and VyOS route toward home gateway `.1`. |
| VMnet2 `10.10.20.0/24` | All nodes `ens37` | OVN **Geneve encapsulation** (normally UDP 6081): carries tenant frames between computes and controller gateways. | **No**. |
| VMnet3 `10.10.30.0/24` | All nodes `ens38` | Ceph MON access and OSD/client data. | **No**. |
| VMnet4 `172.16.50.0/24` | Controller `ens39` (no IP), VyOS `eth1` `.1`, Windows `.254` | Flat external provider network and floating IPs. Kolla attaches controller NIC to `br-ex`. | VyOS is the gateway **for the OpenStack external subnet**; controller host NIC has no IP or gateway. |
...
MAC addresses to the CentOS interface names. `ctrl01` has `ens33`, `ens37`,
`ens38`, and `ens39`; these names are example mappings on the other nodes
until you check their own MACs. Only controllers require `ens39`. The MAC
mapping—not an example name—is authoritative. Use **VMXNET3** adapters for
...
| VM | Planned static VMnet0 IP (set in Section 9) | OpenStack role | Ceph role |
|---|---:|---|---|
...
The column headings show the requested Linux interface names. Your `ctrl01`
has `ens33` for VMnet0; the VMnet assignments of its other three NICs still
need MAC verification. On `ctrl02–03` and `compute01–03`, all interface names
and VMware network associations must be checked separately before running
their example commands or deploying Kolla.

| Host | `ens33` VMnet0 management | `ens37` VMnet2 tunnel | `ens38` VMnet3 Ceph | `ens39` VMnet4 provider |
|---|---:|---:|---:|---|
...
Before using `.49`, `.50–.55`, `.60`, or the floating pool, reserve/exclude
the addresses that belong to a DHCP-served network from its DHCP server and
verify that the planned static addresses are unused. The floating pool is on
the separate VMnet4 network, whose VMware DHCP service is disabled.

Before assigning `.50` on `ctrl01`, while its `ens33` still has DHCP `.180`,
run this duplicate-address check **on `ctrl01`**. Do this before the VIP and
VyOS `.60` are configured as well:

```bash
sudo dnf install -y iputils
sudo arping -D -I ens33 -c 3 192.168.0.49
sudo arping -D -I ens33 -c 3 192.168.0.50
sudo arping -D -I ens33 -c 3 192.168.0.60
```

Expected result for an unused address is zero replies and exit status 0. A
powered-off device cannot answer an ARP check. From Windows, also check your
router's DHCP lease table and use `ping` followed by `arp -a`; reserving the
addresses in the router is still required. If the VIP or `.60` already exists,
skip its unused-address check and confirm its current legitimate owner.
...
- Bridge to: the physical Ethernet/Wi-Fi adapter that reaches
  `192.168.0.1`.
- If `192.168.0.50-55` are already configured on the six CentOS guests, keep
  those assignments. Otherwise assign them **inside CentOS** using the six
  host-specific commands in Phase 4 (Section 9). The VMware Virtual Network
  Editor selects the bridged network; it does not assign a guest's IP address.
...
The **intended mapping**, which requires a VMware MAC check, is:

```text
ens33  -> NIC 1 -> VMnet0
ens37  -> NIC 2 -> VMnet2
ens38  -> NIC 3 -> VMnet3
ens39  -> NIC 4 -> VMnet4 (controllers only)
```

**Your observed `ctrl01` NICs:** the interface with working LAN DHCP address
`192.168.0.180/24` is `ens33`, MAC `00:0c:29:02:bd:df`. The other visible
interfaces are `ens37` (`00:0c:29:02:bd:e9`), `ens38`
(`00:0c:29:02:bd:f3`), and `ens39` (`00:0c:29:02:bd:fd`). Compare those
three MACs with VMware VM Settings before deciding which is VMnet2, VMnet3,
or VMnet4. `ip a` by itself does **not** prove that mapping. Likewise, do not
assume that `ens33`/`ens37`/`ens38`/`ens39` exist on any of your other
VMs. The Kolla globals, firewall, and troubleshooting commands later in this
guide must be adjusted to the verified interface names on **every** host
before deployment. Match the following MACs under **VM Settings → Network
Adapter → Advanced** against each adapter's **Custom: VMnet** selection:

| `ctrl01` Linux NIC | MAC from your `ip a` | Intended VMware network (confirm) |
|---|---|---|
| `ens33` | `00:0c:29:02:bd:df` | VMnet0 management; currently DHCP `.180/24` |
| `ens37` | `00:0c:29:02:bd:e9` | VMnet2 tunnel, if its VMware adapter shows VMnet2 |
| `ens38` | `00:0c:29:02:bd:f3` | VMnet3 Ceph, if its VMware adapter shows VMnet3 |
| `ens39` | `00:0c:29:02:bd:fd` | VMnet4 provider, if its VMware adapter shows VMnet4 |

If a MAC points to another VMnet, fix the VMware adapter connection or change
the Linux device name everywhere it is used for that role. `ip a` cannot
identify VMware VMnet membership by itself.
...
**Run at each VM's VMware console as a sudo-capable OS account.** On your
`ctrl01`, VMnet0 management is **`ens33`**, currently using DHCP address
`192.168.0.180/24`. Its other interfaces must be matched to VMnet2/3/4 by
MAC address. The `ens33`/`ens37`/`ens38`/`ens39` names in the blocks for
the *other* five nodes are examples only: verify those nodes separately and
substitute their real device names before running their blocks.

**First configure VMnet0 management on each CentOS guest.** VMware Workstation
attaches vNIC 1 to bridged VMnet0; NetworkManager **inside each CentOS VM**
sets its static IP. Modify that NIC's **existing active profile** on all six
hosts; adding another management profile can create a competing DHCP/default
route. On `ctrl01` the NIC is `ens33`; confirm that name by MAC on the other
five nodes before using their blocks. Only VMnet0 gets a default gateway.
Make changes at the **VMware console** because activating a profile disconnects
any SSH session to its old IP.

Before running a host block, verify which NIC VMware attaches to VMnet0.
On `ctrl01`, inspect the existing `ens33` connection and its current default
gateway. This guide assumes the home gateway is `192.168.0.1`. **If the
current route shows a different gateway, stop and correct the gateway in the
whole guide**, including the VyOS configuration, before running any block.
Reserve/exclude `192.168.0.50` from the home-router DHCP pool and confirm
through its lease/address inventory that `.50` is unused:

```bash
hostname; nmcli -f GENERAL.DEVICE,GENERAL.HWADDR,GENERAL.CONNECTION device show
ip -4 -br addr show ens33; ip -4 route show default
nmcli -f NAME,UUID,DEVICE connection show --active
```

On `ctrl01`, `nmcli device show ens33` reveals the **active** connection UUID;
`nmcli connection modify uuid` changes that exact DHCP profile to a static
address, and `connection up uuid` activates it. `192.168.0.50/24` means
address `.50` with mask `255.255.255.0`. The DNS list uses the home router
and a public resolver;
if your home router does not provide DNS, use a reachable DNS server instead.
If looking up the active UUID fails or shows no UUID, stop: find and activate
the correct existing `ens33` profile from the VMware console before issuing
any `connection modify` command. Complete each block one command at a time;
do not continue past an error.
The extra NICs on VMnet2/3/4 have no gateway or DNS. Do not put a host IP on
the verified provider device. If the old auto-created profile later reclaims
a NIC, disable that **old** profile only after confirming the new one works.
The `os-tunnel`, `os-storage`, and `os-provider` **add** commands are for the
first run: if one of those profiles already exists on that host, skip its
`add` command and verify its saved address using `nmcli connection show
<profile-name>` before activating it again.

### 9.1 `ctrl01` only — 192.168.0.50; tunnel .11; Ceph .11

**First, set only the management IP on the active `ens33` connection.** The
existing `192.168.0.180` is a DHCP lease and will be replaced when the
connection comes up again. Leave the current IPv6 configuration as it is.
Use the VMware console and confirm the profile UUID is nonempty before the
`modify` command:

```bash
sudo hostnamectl set-hostname ctrl01
MGMT_UUID="$(nmcli -g GENERAL.CON-UUID device show ens33)"
nmcli -f connection.id,connection.interface-name,ipv4.method,ipv4.addresses,ipv4.gateway,ipv6.method connection show uuid "$MGMT_UUID"
sudo nmcli connection modify uuid "$MGMT_UUID" connection.interface-name ens33 ipv4.method manual ipv4.addresses 192.168.0.50/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no connection.autoconnect yes
sudo nmcli connection up uuid "$MGMT_UUID" ifname ens33
ip -4 -br address show ens33
ip -4 route show default
ping -c 3 192.168.0.1
```

`ens33` must now show **only** `192.168.0.50/24` for IPv4; the default IPv4
route must be via `192.168.0.1 dev ens33`. If `.180` remains or the gateway
is unreachable, fix the active profile at the console before moving on.

**Then configure the three other NICs only after matching their MAC addresses
to VMnet2 (tunnel), VMnet3 (Ceph), and VMnet4 (provider) in VMware.** If that
check confirms the order `ens37` → VMnet2, `ens38` → VMnet3, and `ens39` →
VMnet4, run:

```bash
sudo nmcli connection add type ethernet ifname ens37 con-name os-tunnel ipv4.method manual ipv4.addresses 10.10.20.11/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens38 con-name os-storage ipv4.method manual ipv4.addresses 10.10.30.11/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens39 con-name os-provider ipv4.method disabled ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up os-tunnel
sudo nmcli connection up os-storage
sudo nmcli connection up os-provider
```

If the VMware MAC mapping is different, replace `ens37`/`ens38`/`ens39` in
those commands with the verified names. Do not run them just from interface
number order.

**For Sections 9.2–9.6:** On each named host, first verify that its
`ens33` MAC really belongs to VMnet0 and that the active profile UUID is
nonempty. These commands modify the existing management profile, preserving
IPv6. They do not create a second competing DHCP profile. Run only that
host's block at its VMware console and check the gateway and target address
before configuring the other NICs. If a VM has different Linux device names,
replace them everywhere for that VM and update Kolla's host variables.
...
MGMT_UUID="$(nmcli -g GENERAL.CON-UUID device show ens33)"
nmcli -f connection.id,connection.interface-name,ipv4.method,ipv4.addresses,ipv4.gateway,ipv6.method connection show uuid "$MGMT_UUID"
sudo nmcli connection modify uuid "$MGMT_UUID" connection.interface-name ens33 ipv4.method manual ipv4.addresses 192.168.0.51/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no connection.autoconnect yes
sudo nmcli connection up uuid "$MGMT_UUID" ifname ens33
ip -4 -br address show dev ens33
ip -4 route show default
```

**On `ctrl02`:** Continue only if `ens33` now has `192.168.0.51/24`,
the IPv4 default route uses `192.168.0.1 dev ens33`, and the
VMnet2/3/4 interface MACs match the intended VMware attachments.

```bash
sudo nmcli connection add type ethernet ifname ens37 con-name os-tunnel ipv4.method manual ipv4.addresses 10.10.20.12/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens38 con-name os-storage ipv4.method manual ipv4.addresses 10.10.30.12/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens39 con-name os-provider ipv4.method disabled ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up os-tunnel
sudo nmcli connection up os-storage
sudo nmcli connection up os-provider
...
MGMT_UUID="$(nmcli -g GENERAL.CON-UUID device show ens33)"
nmcli -f connection.id,connection.interface-name,ipv4.method,ipv4.addresses,ipv4.gateway,ipv6.method connection show uuid "$MGMT_UUID"
sudo nmcli connection modify uuid "$MGMT_UUID" connection.interface-name ens33 ipv4.method manual ipv4.addresses 192.168.0.52/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no connection.autoconnect yes
sudo nmcli connection up uuid "$MGMT_UUID" ifname ens33
ip -4 -br address show dev ens33
ip -4 route show default
```

**On `ctrl03`:** Continue only if `ens33` now has `192.168.0.52/24`,
the IPv4 default route uses `192.168.0.1 dev ens33`, and the
VMnet2/3/4 interface MACs match the intended VMware attachments.

```bash
sudo nmcli connection add type ethernet ifname ens37 con-name os-tunnel ipv4.method manual ipv4.addresses 10.10.20.13/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens38 con-name os-storage ipv4.method manual ipv4.addresses 10.10.30.13/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens39 con-name os-provider ipv4.method disabled ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up os-tunnel
sudo nmcli connection up os-storage
sudo nmcli connection up os-provider
...
MGMT_UUID="$(nmcli -g GENERAL.CON-UUID device show ens33)"
nmcli -f connection.id,connection.interface-name,ipv4.method,ipv4.addresses,ipv4.gateway,ipv6.method connection show uuid "$MGMT_UUID"
sudo nmcli connection modify uuid "$MGMT_UUID" connection.interface-name ens33 ipv4.method manual ipv4.addresses 192.168.0.53/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no connection.autoconnect yes
sudo nmcli connection up uuid "$MGMT_UUID" ifname ens33
ip -4 -br address show dev ens33
ip -4 route show default
```

**On `compute01`:** Continue only if `ens33` now has `192.168.0.53/24`,
the IPv4 default route uses `192.168.0.1 dev ens33`, and the
VMnet2/3 interface MACs match the intended VMware attachments. This compute
does not have a VMnet4 interface in the centralized-gateway design.

```bash
sudo nmcli connection add type ethernet ifname ens37 con-name os-tunnel ipv4.method manual ipv4.addresses 10.10.20.21/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens38 con-name os-storage ipv4.method manual ipv4.addresses 10.10.30.21/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up os-tunnel
sudo nmcli connection up os-storage
ip -4 -br address; ip -4 route
```
...
MGMT_UUID="$(nmcli -g GENERAL.CON-UUID device show ens33)"
nmcli -f connection.id,connection.interface-name,ipv4.method,ipv4.addresses,ipv4.gateway,ipv6.method connection show uuid "$MGMT_UUID"
sudo nmcli connection modify uuid "$MGMT_UUID" connection.interface-name ens33 ipv4.method manual ipv4.addresses 192.168.0.54/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no connection.autoconnect yes
sudo nmcli connection up uuid "$MGMT_UUID" ifname ens33
ip -4 -br address show dev ens33
ip -4 route show default
```

**On `compute02`:** Continue only if `ens33` now has `192.168.0.54/24`,
the IPv4 default route uses `192.168.0.1 dev ens33`, and the
VMnet2/3 interface MACs match the intended VMware attachments. This compute
does not have a VMnet4 interface in the centralized-gateway design.

```bash
sudo nmcli connection add type ethernet ifname ens37 con-name os-tunnel ipv4.method manual ipv4.addresses 10.10.20.22/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens38 con-name os-storage ipv4.method manual ipv4.addresses 10.10.30.22/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up os-tunnel
sudo nmcli connection up os-storage
ip -4 -br address; ip -4 route
```
...
MGMT_UUID="$(nmcli -g GENERAL.CON-UUID device show ens33)"
nmcli -f connection.id,connection.interface-name,ipv4.method,ipv4.addresses,ipv4.gateway,ipv6.method connection show uuid "$MGMT_UUID"
sudo nmcli connection modify uuid "$MGMT_UUID" connection.interface-name ens33 ipv4.method manual ipv4.addresses 192.168.0.55/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no connection.autoconnect yes
sudo nmcli connection up uuid "$MGMT_UUID" ifname ens33
ip -4 -br address show dev ens33
ip -4 route show default
```

**On `compute03`:** Continue only if `ens33` now has `192.168.0.55/24`,
the IPv4 default route uses `192.168.0.1 dev ens33`, and the
VMnet2/3 interface MACs match the intended VMware attachments. This compute
does not have a VMnet4 interface in the centralized-gateway design.

```bash
sudo nmcli connection add type ethernet ifname ens37 con-name os-tunnel ipv4.method manual ipv4.addresses 10.10.20.23/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection add type ethernet ifname ens38 con-name os-storage ipv4.method manual ipv4.addresses 10.10.30.23/24 ipv4.never-default yes ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up os-tunnel
sudo nmcli connection up os-storage
ip -4 -br address; ip -4 route
```
...
ip -4 -br address
ip -4 route show default
nmcli -f GENERAL.DEVICE,GENERAL.CONNECTION device show
nmcli -f NAME,UUID,DEVICE connection show --active
...
Each host must show its own `192.168.0.50–55/24` on its **verified VMnet0
interface** (on `ctrl01`, this is `ens33`), a working `ping 192.168.0.1`, and
**one** IPv4 default route via that interface. On a controller, its **verified
provider interface** must be up without an IPv4 address.
...
If `getenforce` does not report `Disabled`, reboot once more:

```bash
sudo reboot
```

After reconnecting to that same host, verify:
...
```bash
sudo firewall-cmd --permanent --zone=trusted --change-interface=ens33
sudo firewall-cmd --permanent --zone=trusted --change-interface=ens37
sudo firewall-cmd --permanent --zone=trusted --change-interface=ens38
...
Identify the provider interface for each host using its VMware MAC address.
That interface must be `UP`, with no host IP address. On your `ctrl01`, only
use `ens39` for this check **if VMware confirms its MAC is on VMnet4**.
...
> [!IMPORTANT]
> The values below use the requested **target mapping**: `ens33` for VMnet0,
> `ens37` for VMnet2, `ens38` for VMnet3, and `ens39` for controller VMnet4.
> Your `ctrl01` management NIC is confirmed as `ens33`; its other VMware
> network assignments and every other node's NIC names still require the
> MAC checks in Section 7. If any host differs, override the relevant
> variables in inventory `host_vars`/`group_vars` before running
> `bootstrap-servers`, `prechecks`, or `deploy`. Never deploy an unverified
> global mapping.
...
# 10.10.30.0/24 route on ens38, so do not add storage_interface here.

# Only controller/network nodes have ens39 in this centralized-gateway design.
neutron_external_interface: "ens39"
...
The VMnet3/`ens38` configuration from Phase 4 remains mandatory. Kolla does
...
table selects `ens38` for `10.10.30.0/24`.
...
For example, if VMware confirms that `ctrl02` uses names different from the
table, put its **verified** management, tunnel, and provider interface names
in `/opt/openstack/inventory/host_vars/ctrl02.yml` with keys
`network_interface`, `api_interface`, `tunnel_interface`, and
`neutron_external_interface`. Repeat for each host that differs. The storage
path is selected by its connected `10.10.30.0/24` route, so confirm its
separate Ceph interface has the planned `.11–.23` address. On `ctrl01`, inspect
the common values and any inventory overrides before deployment:

```bash
grep -E '^(network_interface|api_interface|tunnel_interface|neutron_external_interface):' /etc/kolla/globals.yml
for host in ctrl01 ctrl02 ctrl03 compute01 compute02 compute03; do
  echo "Inventory overrides for $host"
  ansible-inventory -i /opt/openstack/inventory/multinode --host "$host" |
    python -c 'import json,sys; x=json.load(sys.stdin); print({k:x.get(k) for k in ("network_interface","api_interface","tunnel_interface","neutron_external_interface")})'
done
```

In the inventory output, `None` means no per-host override; use the common
value from `globals.yml` for that host. Compare the effective mapping to its
VMware MAC check, then let Kolla `prechecks` validate the deployment roles.
The provider global does not require an `ens39` NIC on computes; with this
centralized design it is **used only on the controllers** in `[network]`.
...
networks on computes are disabled, computes do not need `ens39`.

This produces a centralized north-south path:

```text
instance -> compute OVN -> Geneve -> controller OVN gateway
         -> br-ex/ens39 -> VMnet4 -> VyOS -> VMnet0 -> Internet
```
...
sudo ceph auth get-or-create client.glance \
  ...
  mgr 'profile rbd pool=images' >/dev/null

sudo ceph auth get-or-create client.cinder \
  mon 'profile rbd' \
  osd 'profile rbd pool=volumes, profile rbd pool=vms, profile rbd-read-only pool=images' \
  mgr 'profile rbd pool=volumes, profile rbd pool=vms' >/dev/null

sudo ceph auth get-or-create client.cinder-backup \
  mon 'profile rbd' \
  osd 'profile rbd pool=backups' \
  mgr 'profile rbd pool=backups' >/dev/null

sudo ceph auth get-or-create client.nova \
  mon 'profile rbd' \
  osd 'profile rbd pool=vms, profile rbd-read-only pool=images' \
  mgr 'profile rbd pool=vms' >/dev/null
```

The output of `ceph auth get-or-create` includes key material, so redirect it
away from terminal recordings. Section 17 exports the same identities to the
required files with restrictive permissions.
...
| Attach `ens39` to `br-ex` and map `physnet1` | No | No | **Kolla deploy from `ctrl01`** |
...
```powershell
$FloatingIp = Read-Host 'Enter the floating IP printed in Section 20.5'
ping $FloatingIp
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.ssh" | Out-Null
scp cloudadmin@192.168.0.50:.ssh/openstack-lab "$env:USERPROFILE\.ssh\openstack-lab"
ssh -i "$env:USERPROFILE\.ssh\openstack-lab" "cirros@$FloatingIp"
```

Run the `scp` command on Windows as the intended Windows user; it copies the
private key generated on `ctrl01` in Section 20.3 over SSH to that user's
profile. Keep the copied private key readable only by that user and delete it
when the lab no longer needs it. Alternatively, run the SSH test from another
trusted Linux VM attached to VMnet4. If ping fails, check Neutron security
rules, the floating-IP association, and the Windows VMnet4 address before SSH.
...
```bash
for host in ctrl01 ctrl02 ctrl03; do
  ssh "$host" "ip -4 -br address show ens33 | grep 192.168.0.49 || true"
done
```
...
following prompt checks only the name's **syntax**, so personally confirm
that the daemon shown belongs to `compute01` before submitting it. During
the pause, verify `ceph -s`
...
### 26.1 `ens33`/`ens37` names do not match
...
```bash
for host in ctrl01 ctrl02 ctrl03; do
  ssh "$host" 'ip -br link show ens39; ip -br address show ens39'
done
```
...
Common causes are a wrong VMnet4 attachment, missing `ens39` bridge, incorrect
...
- [ ] `ctrl01` has static `192.168.0.50/24` on `ens33` and no lingering DHCP `.180`.
- [ ] The other five hosts have their assigned `.51–.55/24` management IPs.
...
- [NetworkManager — configure a static Ethernet address, gateway and DNS](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/configuring_and_managing_networking/configuring-an-ethernet-connection)
- [NetworkManager — select a connection by UUID and activate it on a specific device](https://networkmanager.dev/docs/api/latest/nmcli.html)
...
The planned `192.168.0.50-55` addresses serve management/Ansible/API;
`ctrl01` moves from DHCP `.180` to static `.50` on `ens33` in Section 9.1.
Three added isolated VMware networks separate OVN tunnels, Ceph
