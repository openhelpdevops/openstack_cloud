...
**Validated design date:** 21 September 2026  
...
6. Cisco sends the translated traffic through VMnet0 to your existing gateway
   `192.168.0.10`, and then to the Internet.
...
![Six-node OpenStack VMware Cisco Ceph architecture](openstack_6_node_vmware_cisco_ceph_architecture.png)
...
flowchart TB
    ...
    Internet["Internet"]:::upstream --> Home["Home/LAN gateway<br/>192.168.0.10"]:::upstream
...
| Existing Internet gateway | `192.168.0.10` |
...
ping 192.168.0.10 source 192.168.0.60
...
```text
default via 192.168.0.10 dev ens160
...
The Kolla `storage` group identifies where Cinder volume/backup containers run;
it does not mean Ceph OSDs run on those controllers. Ceph OSDs remain on the
three computes and are managed separately by `cephadm`.
...
ansible -i /opt/openstack/inventory/multinode all \
  -a 'sudo docker info --format "Server={{.ServerVersion}} Driver={{.Driver}}"'
```

If the second command's braces conflict with your Ansible/shell version, run
this simpler check:
...
If Ansible interprets the Docker template braces, SSH to each node and run
`sudo docker ps` directly.
...
- [ ] VMnet0 remains bridged and all nodes reach `192.168.0.10`.
...
- [ ] Cisco default route points to `192.168.0.10`.
...
- [OpenStack release status — 2026.1 Gazpacho](https://releases.openstack.org/)
