...
**Revision:** Version 4 — exact per-host network commands, self-contained diagrams, Horizon walkthrough, and three 35 GB OSDs; `ens33`/`ens37`/`ens38`/`ens39` interface update  
...
**Current `ctrl01` evidence:** `ens33` has the DHCP address
`192.168.0.180/24`. Its `ens37`, `ens38`, and `ens39` interfaces have no IP
address yet. This guide maps them to VMnet2, VMnet3, and VMnet4 respectively
**only after** you match their MAC addresses against the VMware adapter
settings in Section 7. The same Linux names on the other five CentOS VMs
remain an assumption until you check each VM separately. Section 9.1 contains
the commands to set `192.168.0.50/24` on `ctrl01`'s existing `ens33` profile.
...
MAC addresses to the CentOS interface names. The observed names on `ctrl01`
are `ens33`, `ens37`, `ens38`, and `ens39`; the other five nodes use these same
names in examples only after verifying their own MAC mappings. The MAC
...
The interface names below are observed on `ctrl01`. They are examples for
`ctrl02`, `ctrl03`, and the computes until their own VMware adapters are
matched by MAC address. Even on `ctrl01`, confirm which VMware VMnet each
observed MAC belongs to **before** assigning tunnel, Ceph, or provider roles.
...
- Bridge to: the physical Ethernet/Wi-Fi adapter that reaches
  `192.168.0.1`.
- Keep the planned management addresses `192.168.0.50-55` on this LAN.
  `ctrl01` is currently DHCP `192.168.0.180/24`; Section 9.1 converts its
  active `ens33` profile to the planned static `.50/24` address. If another
  host is on DHCP, assign its planned address before Kolla deployment too.
...
Target mapping in this guide, **conditional on matching each VMware MAC**:
...
`ens39` merely because this guide uses that name.

The `ctrl01` interface MACs you reported are:

| `ctrl01` interface | MAC seen in `ip a` | VMware network to confirm in VM Settings |
|---|---|---|
| `ens33` | `00:0c:29:02:bd:df` | VMnet0 management; currently DHCP `.180/24` |
| `ens37` | `00:0c:29:02:bd:e9` | VMnet2 tunnel, if this MAC belongs to vNIC 2 |
| `ens38` | `00:0c:29:02:bd:f3` | VMnet3 Ceph, if this MAC belongs to vNIC 3 |
| `ens39` | `00:0c:29:02:bd:fd` | VMnet4 provider, if this MAC belongs to vNIC 4 |

An `ip a` listing identifies interfaces and addresses; it does **not** show
which VMnet a NIC is attached to. In VMware Workstation, open `ctrl01` →
**VM Settings** → each **Network Adapter** → **Advanced** and compare the MAC
and **Custom: VMnet...** setting. Repeat the comparison on the other five
CentOS VMs; computes have no `ens39` in this design. Correct any mismatch in
the examples and `/etc/kolla/globals.yml` before starting deployment.
...
`ens33` = VMnet0, `ens37` = VMnet2, `ens38` = VMnet3, and controllers'
`ens39` = VMnet4. Substitute your verified Linux device names if different.

On `ctrl01`, `ens33` currently has DHCP address `192.168.0.180/24`.
Section 9.1 changes **that active management profile** to static
`192.168.0.50/24`. On the other hosts, leave a management connection alone
only if it already has its planned static `.51–.55/24` address; otherwise set
its assigned address before continuing. The only IPv4 default route on each
host should be through VMnet0. This guide assumes your existing gateway is
`192.168.0.1`; confirm with `ip -4 route show default` on the current DHCP
connection. If your real gateway differs, update the IP plan and gateway
examples consistently before proceeding. To check an individual host
...
There is no gateway or DNS on the new profiles. Do not put an IP on `ens39`.

**Gate:** Do not apply any `ens37`, `ens38`, or `ens39` example unless its MAC
matches the intended VMware network from Section 7. On any host with a
different Linux interface name, replace the name consistently in that host's
profile and in Kolla inventory variables. You will change `ctrl01`'s working
management IP, so perform Section 9.1 at the **VMware console**; an existing
SSH session to `.180` will disconnect when the new address takes effect.

### 9.1 `ctrl01` only — 192.168.0.50; tunnel .11; Ceph .11

**On `ctrl01` only, at its VMware console:** Check that `ens33` is the active
VMnet0 management NIC and note its active connection. Verify that the
current IPv4 default gateway is `192.168.0.1`. Reserve or exclude `.50` from
the home router's DHCP pool, and confirm that no other device uses `.50`
using the router's leases and address records. Your current `.180` is assigned
by DHCP; it is not the target address.

```bash
ip -4 -br address show dev ens33
ip -4 route show default
nmcli -f GENERAL.DEVICE,GENERAL.HWADDR,GENERAL.CONNECTION device show ens33
nmcli -f NAME,UUID,DEVICE connection show --active
```

**Still on `ctrl01`, while connected through the VMware console:** Modify the
*existing active profile*, instead of adding another profile to `ens33`.
The commands select that profile by its unique NetworkManager UUID. If
`MGMT_UUID` is empty or `--`, stop and check the active `ens33` profile.
These commands set `.50/24`, default gateway `.1`, and the example DNS
resolvers. Confirm that your router at `.1` provides DNS; if it does not,
replace `192.168.0.1` in `ipv4.dns` with a working resolver. The commands
do not change your current IPv6 configuration.

```bash
MGMT_UUID="$(nmcli -g GENERAL.CON-UUID device show ens33)"
printf 'Active ens33 connection UUID: %s\n' "$MGMT_UUID"
...
Check that `ens33` now has **`192.168.0.50/24`**, `.180` is gone, and there
is exactly one IPv4 default route via `192.168.0.1 dev ens33`. If `.50` does
not appear or the gateway cannot be reached, correct the existing profile at
the console before continuing. The profile will reconnect automatically on
reboot. Continue with the remaining `ctrl01` interfaces only after verifying
their VMware MAC and VMnet association in Section 7.
...
`ip -4 addr show ens39` must return no IPv4 address; the link must be UP.
...
- [ ] `ctrl01` `ens33` is static `192.168.0.50/24` with no lingering DHCP `.180` address.
- [ ] The other five nodes have their planned `.51–.55/24` management addresses.
...
- [Red Hat Enterprise Linux 10 — configure an existing NetworkManager Ethernet connection with a static address](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/configuring_and_managing_networking/configuring-an-ethernet-connection)
- [NetworkManager nmcli reference — connection activation](https://networkmanager.dev/docs/api/latest/nmcli.html)
...
`ctrl01` moves from DHCP `.180` to static `.50` in Section 9.1. Three added
isolated VMware networks separate OVN tunnels, Ceph
