...
The interface names in the column headings below were examples from the first
draft. Your `ctrl01` has `ens33` for VMnet0, and its other NICs still need MAC
verification. You **must verify interface names on every host** before using
any network commands or the Kolla globals example.
...
The earlier example mapping (seen on some VMware VM hardware versions) was:
...
**Your actual `ctrl01` differs:** the interface with working LAN DHCP address
...
assume that `ens160`/`ens192`/`ens224`/`ens256` exist on any of your other
...
MAC address. The `ens160`/`ens192`/`ens224`/`ens256` names in the blocks for
...
sets the static IP. On `ctrl01`, modify **the existing active profile of
`ens33`**; do not create a new connection for a nonexistent `ens160` NIC.
The blocks for the other hosts still need their own NIC names checked before
use. Only VMnet0 gets a default gateway. Make changes at the **VMware console**:
activating a profile can temporarily disconnect SSH.
...
gateway. This guide expects the existing home gateway to be `192.168.0.1`;
if `ip -4 route show default` shows a different gateway, use that real gateway
instead. Reserve/exclude `192.168.0.50` from your home-router DHCP pool and
ensure that nothing else uses `.50` before changing this VM:
...
On `ctrl01`, `nmcli device show ens33` reveals the **active** connection name;
`nmcli connection modify` changes that existing DHCP profile to a static
address, and `connection up` activates it. `192.168.0.50/24` means address `.50` with mask
`255.255.255.0`. The DNS list uses the home router and a public resolver;
...
printf 'Active ens33 connection: %s\n' "$CON"
sudo nmcli connection modify "$CON" ipv4.method manual ipv4.addresses 192.168.0.50/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no connection.autoconnect yes
...
nmcli connection show mgmt-ens160 >/dev/null 2>&1 || sudo nmcli connection add type ethernet ifname ens160 con-name mgmt-ens160 connection.autoconnect no
sudo nmcli connection modify mgmt-ens160 connection.interface-name ens160 ipv4.method manual ipv4.addresses 192.168.0.51/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up mgmt-ens160
...
nmcli connection show mgmt-ens160 >/dev/null 2>&1 || sudo nmcli connection add type ethernet ifname ens160 con-name mgmt-ens160 connection.autoconnect no
sudo nmcli connection modify mgmt-ens160 connection.interface-name ens160 ipv4.method manual ipv4.addresses 192.168.0.52/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up mgmt-ens160
...
nmcli connection show mgmt-ens160 >/dev/null 2>&1 || sudo nmcli connection add type ethernet ifname ens160 con-name mgmt-ens160 connection.autoconnect no
sudo nmcli connection modify mgmt-ens160 connection.interface-name ens160 ipv4.method manual ipv4.addresses 192.168.0.53/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up mgmt-ens160
...
nmcli connection show mgmt-ens160 >/dev/null 2>&1 || sudo nmcli connection add type ethernet ifname ens160 con-name mgmt-ens160 connection.autoconnect no
sudo nmcli connection modify mgmt-ens160 connection.interface-name ens160 ipv4.method manual ipv4.addresses 192.168.0.54/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up mgmt-ens160
...
nmcli connection show mgmt-ens160 >/dev/null 2>&1 || sudo nmcli connection add type ethernet ifname ens160 con-name mgmt-ens160 connection.autoconnect no
sudo nmcli connection modify mgmt-ens160 connection.interface-name ens160 ipv4.method manual ipv4.addresses 192.168.0.55/24 ipv4.gateway 192.168.0.1 ipv4.dns '192.168.0.1 1.1.1.1' ipv4.never-default no ipv6.method disabled connection.autoconnect yes connection.autoconnect-priority 100
sudo nmcli connection up mgmt-ens160
...
> [!IMPORTANT]
> **Do not deploy this example with its `ens160`, `ens192`, and `ens256`
> values as written.** Your `ctrl01` management NIC is `ens33`. Verify the
> VMnet2 and VMnet4 MAC mappings on `ctrl01` and the interface names on the
> other five hosts. If every host has the same names for each role, update
> the three global interface values below. If names differ by host, use
> Kolla inventory host-specific interface settings. Check the final rendered
> inventory before running `bootstrap-servers`, `prechecks`, or `deploy`.
